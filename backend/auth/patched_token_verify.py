...
def verify_supabase_token(token: str) -> Tuple[bool, Optional[Dict[str, Any]]]:
    ...
    # If development mode is enabled, use simplified verification
    if DEV_MODE:
    ...
    try:
        ...
        try:
            ...
            return True, payload
        except JWTError:  # Changed from InvalidAudienceError
            # Try without audience verification
            logger.warning("Token has invalid audience or other JWT error, attempting without audience check")
            try:
                payload = jwt.decode(
                    token,
                    rsa_key,
